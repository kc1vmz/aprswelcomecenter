...
public class KISSTCPIPClient {
    ...
    public void connect(String host, int port) {
        try {
            ...
            logger.error("Exception caught connecting to KISS port: " + e.getMessage(), e);
        }
        setValid(false);
    }
    ...
    public KISSPacket listen() throws IOException {
        ...
        while (nextControl != KISSControlCode.UI) {
            byte[] repeatBytes =
                    Arrays.copyOfRange(resp, currentPos, currentPos + callsignLength + 1); // here for 7 characters
