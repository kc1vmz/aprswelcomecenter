...
@RestController
@RequestMapping("/api/v1/communication-categories")
public class CommunicationCategoryController {
    @Autowired
    private CommunicationCategoryAccessor accessor;

    @GetMapping
    public Flux<CommunicationCategory> all() {
        return accessor.findAll();
    }

    @GetMapping("/{id}")
    public Mono<CommunicationCategory> one(@PathVariable UUID id) {
        return accessor.findById(id);
    }

    @PostMapping
    public Mono<ResponseEntity<CommunicationCategory>> create(@RequestBody CommunicationCategory value) {
        return accessor.create(value)
                .map(saved -> ResponseEntity.status(HttpStatus.CREATED).body(saved));
    }

    @PutMapping("/{id}")
    public Mono<CommunicationCategory> replace(@PathVariable UUID id, @RequestBody CommunicationCategory value) {
